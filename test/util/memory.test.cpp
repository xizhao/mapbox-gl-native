...
TEST(Memory, Vector) {
    ...
    Map map(std::make_unique<StubRendererFrontend>(test.backend, test.view),
            MapObserver::nullObserver(), {256, 256}, 2, test.fileSource, test.threadPool,
            MapMode::Still);
...
TEST(Memory, Footprint) {
    ...
    for (unsigned i = 0; i < runs; ++i) {
        auto vector = std::make_unique<Map>(
                std::make_unique<StubRendererFrontend>(test.backend, test.view),
                MapObserver::nullObserver(), Size{256, 256}, 2, test.fileSource, test.threadPool,
                MapMode::Still);
    ...
    for (unsigned i = 0; i < runs; ++i) {
        auto raster = std::make_unique<Map>(
                std::make_unique<StubRendererFrontend>(test.backend, test.view),
                MapObserver::nullObserver(), Size{256, 256}, 2, test.fileSource, test.threadPool,
                MapMode::Still);
